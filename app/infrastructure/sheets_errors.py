...
def _is_rate_limited_api_error(
    ex: gspread.exceptions.APIError, text_lower: str
) -> bool:
    response = getattr(ex, "response", None)
...
def map_gspread_exception(ex: Exception) -> Exception:
    ...
    if isinstance(ex, gspread.exceptions.APIError):
        ...
        if (
            "google sheets api has not been used" in text_lower
            or "it is disabled" in text_lower
        ):
    ...
    if isinstance(ex, json.JSONDecodeError):
        return SheetsCredentialsError(
            "El credentials.json no es válido. Revisa el contenido del archivo."
        )
    ...
    if isinstance(ex, AttributeError):
        return SheetsClientError(redactar_texto(str(ex)))
    return SheetsConfigError(redactar_texto(str(ex)))
