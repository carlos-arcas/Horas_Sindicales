...
from app.ui.vistas.main_window_ui_builder import clear_form, create_widgets
...
class MainWindow(QMainWindow):
    ...
    def _create_widgets(self) -> None:
        self.abrir_pdf_check = self.add_persona_button = self.agregar_button = self.alert_banner_label = self.completo_check = self.config_delegada_combo = self.confirm_sync_button = self.confirmar_button = self.confirmation_summary_label = self.conflicts_reminder_label = None
        self.consequence_microcopy_label = self.copy_sync_report_button = self.cuadrante_warning_label = self.delegada_field_error = self.delete_persona_button = self.desde_container = self.desde_input = self.desde_placeholder = self.edit_grupo_button = self.edit_persona_button = None
        self.editar_pdf_button = self.eliminar_button = self.eliminar_huerfana_button = self.eliminar_pendiente_button = self.fecha_field_error = self.fecha_input = self.generar_pdf_button = self.go_to_sync_config_button = self.goto_existing_button = self.hasta_container = None
        self.hasta_input = self.hasta_placeholder = self.health_checks_tree = self.health_summary_label = self.historico_clear_filters_button = self.historico_delegada_combo = self.historico_desde_date = self.historico_details_button = self.historico_details_content = self.historico_empty_state = None
        self.historico_estado_combo = self.historico_hasta_date = self.historico_last_30_button = self.historico_model = self.historico_proxy_model = self.historico_search_input = self.historico_table = self.huerfanas_label = self.huerfanas_model = self.huerfanas_table = None
        self.insertar_sin_pdf_button = self.last_sync_label = self.last_sync_metrics_label = self.main_tabs = self.notas_input = self.opciones_button = self.pendientes_model = self.pendientes_table = self.pending_errors_frame = self.pending_errors_summary = None
        self.pending_filter_warning = self.persona_combo = self.primary_cta_button = self.primary_cta_hint = self.resync_historico_button = self.retry_failed_button = self.review_conflicts_button = self.revisar_ocultas_button = self.simulate_sync_button = self.solicitud_inline_error = None
        self.stepper_context_label = self.stepper_labels = self.sync_button = self.sync_counts_label = self.sync_details_button = self.sync_idempotency_label = self.sync_panel_status = self.sync_progress = self.sync_scope_label = self.sync_source_label = None
        self.sync_status_badge = self.sync_status_label = self.sync_trend_label = self.total_pendientes_label = self.total_preview_input = self.tramo_field_error = self.ver_detalle_button = self.ver_todas_pendientes_button = None
        create_widgets(self)
