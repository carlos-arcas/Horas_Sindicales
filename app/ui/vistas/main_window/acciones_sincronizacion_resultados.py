...
def on_push_now(ventana) -> None:
    if not ventana._sync_service.is_configured():
        ventana.toast.warning("No hay configuración de Google Sheets. Abre Opciones para configurarlo.", title="Sin configuración")
        return
...
def on_push_finished(ventana, summary: SyncSummary) -> None:
    ...
    ventana._refresh_last_sync_label()
    dialogos_sincronizacion.show_sync_summary_dialog(ventana, "Sincronización completada", summary)
...
def set_sync_in_progress(ventana, in_progress: bool) -> None:
    ...
    if in_progress:
        ...
        ventana.statusBar().showMessage("Sincronizando con Google Sheets…")
        ...
        ventana.sync_panel_status.setText("Estado: Pendiente · Sincronizando")
    else:
