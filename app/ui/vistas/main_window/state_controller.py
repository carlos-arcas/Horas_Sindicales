...
from . import state_controller_utils
...
class MainWindow(MainWindowStateActionsMixin, MainWindowStateValidationMixin, MainWindowHealthMixin, QMainWindow):
    ...
    def _apply_help_preferences(self) -> None:
        state_controller_utils.apply_help_preferences(self)

    def _on_help_toggle_changed(self, enabled: bool) -> None:
        state_controller_utils.on_help_toggle_changed(self, enabled)

    def _apply_solicitudes_tooltips(self, enabled: bool | None = None) -> None:
        state_controller_utils.apply_solicitudes_tooltips(self, enabled)
    ...
    def _update_conflicts_reminder(self) -> None:
        state_controller_utils.update_conflicts_reminder(self, logger)
    ...
    def _configure_time_placeholders(self) -> None:
        state_controller_utils.configure_time_placeholders(self, handlers_layout)

    def _normalize_input_heights(self) -> None:
        """Unifica la altura de los campos del formulario de solicitud."""
        state_controller_utils.normalize_input_heights(self, logger)
    ...
    def _configure_operativa_focus_order(self) -> None:
        state_controller_utils.configure_operativa_focus_order(self)

    def _configure_historico_focus_order(self) -> None:
        state_controller_utils.configure_historico_focus_order(self, logger)

    def _status_to_label(self, status: str) -> str:
        return state_controller_utils.status_to_label(status, status_badge)

    def _configure_solicitudes_table(self, table: QTableView) -> None:
        state_controller_utils.configure_solicitudes_table(table)
