...
def test_quality_gate_script_contract() -> None:
    ...
    assert "pytest --cov=app --cov-report=term-missing --cov-fail-under=85" in content, (
        "quality_gate.bat debe ejecutar pytest con cobertura y umbral"
    )
    assert content.index("python scripts/preflight_tests.py") < content.index("pytest --cov=app --cov-report=term-missing --cov-fail-under=85"), (
