...
def test_quality_gate_script_contract() -> None:
    ...
    assert "set \"min_coverage=85\"" in content, (
        "quality_gate.bat debe declarar umbral minimo de cobertura"
    )
    assert "--cov-fail-under=%min_coverage%" in content, (
        "quality_gate.bat debe ejecutar pytest con cobertura y umbral"
    )
    assert content.index("python scripts/preflight_tests.py") < content.index("--cov-fail-under=%min_coverage%"), (
        "quality_gate.bat debe correr preflight antes de pytest"
    )
    assert "[quality] min_coverage=" in content, (
        "quality_gate.bat debe registrar el criterio explicito de cobertura"
    )
