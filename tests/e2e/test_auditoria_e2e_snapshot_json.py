...
def test_auditoria_e2e_snapshot_json_dry_run(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    actual = normalizar_json(resultado.contenido_json)

    golden = Path(__file__).resolve().parents[1] / "golden" / "auditoria_e2e.json"
