...
from tests.utilidades.normalizar_reportes import guardar_golden, normalizar_markdown
...
def test_auditoria_e2e_snapshot_markdown_dry_run(tmp_path: Path) -> None:
    _seed(tmp_path)
    auditor = AuditarE2E(reloj=RelojFijo(), fs=SistemaArchivosLocal(), repo_info=RepoTmp(tmp_path), hasher=Sha256Hasher())

    resultado = auditor.ejecutar(RequestAuditoriaE2E(dry_run=True, id_auditoria="AUD-TEST-0003"))
    actual = normalizar_markdown(resultado.contenido_md)

    golden = Path(__file__).resolve().parents[1] / "golden" / "auditoria_e2e.md"
    if os.getenv("UPDATE_GOLDEN") == "1":
        guardar_golden(golden, actual)
