...
def test_confirmar_pdf_por_filtro_none_incluye_varias_delegadas(
    connection, tmp_path: Path
) -> None:
    persona_repo = RepositorioPersonasSQLite(connection)
    solicitud_repo = SolicitudRepositorySQLite(connection)
    p1 = persona_repo.create(
        Persona(
            id=None,
            nombre="A",
            genero="F",
    ...
    ruta, ids, resumen = use_case._coordinador_confirmacion_pdf.confirmar_y_generar_pdf_por_filtro(
...
def test_confirmar_pdf_por_filtro_delegada_aplica_subset(
    connection, tmp_path: Path
) -> None:
    persona_repo = RepositorioPersonasSQLite(connection)
    solicitud_repo = SolicitudRepositorySQLite(connection)
    ...
    ruta, ids, _ = use_case._coordinador_confirmacion_pdf.confirmar_y_generar_pdf_por_filtro(
...
def test_confirmar_pdf_por_filtro_sin_pendientes_devuelve_warning(
    connection, tmp_path: Path
) -> None:
    persona_repo = RepositorioPersonasSQLite(connection)
    solicitud_repo = SolicitudRepositorySQLite(connection)
