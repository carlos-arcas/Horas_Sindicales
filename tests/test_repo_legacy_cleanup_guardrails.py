...
def test_no_reaparecen_referencias_a_stack_web_ajeno() -> None:
    ...
    for rel in _iter_repo_files():
        if rel in ALLOWLIST_TEXT or rel.parts[:1] == ('_backstage',):
            continue
...
def test_no_reaparecen_archivos_o_docs_con_naming_legacy_evidente() -> None:
    offenders: list[str] = []
    for rel in _iter_repo_files():
        if rel.parts[:1] == ('_backstage',) or rel in ALLOWLIST_NOMBRES:
            continue
        for patron in PATRONES_NOMBRE_LEGACY:
            if patron.search(rel.name):
                ...
                break
    assert not offenders, (
        'Se detectaron archivos legacy/placeholders fuera del archivo histórico permitido:\n'
        + '\n'.join(offenders)
    )


def test_entrypoints_y_scripts_canonicos_siguen_existiendo() -> None:
    faltantes = [ruta.as_posix() for ruta in ENTRYS_CANONICOS if not (ROOT / ruta).exists()]
    assert not faltantes, 'Faltan entrypoints o scripts canónicos: ' + ', '.join(faltantes)
