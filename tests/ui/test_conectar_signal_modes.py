...
def test_conectar_signal_modo_estricto_lanza_runtime_error(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WIRING_STRICT", "true")
    monkeypatch.delenv("CI", raising=False)

    with pytest.raises(RuntimeError, match=r"_on_confirmar.*builder:confirmar"):
        conectar_signal(VentanaSinHandler(), SignalFalso(), "_on_confirmar", contexto="builder:confirmar")


def test_conectar_signal_modo_runtime_no_lanza_y_loguea(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
