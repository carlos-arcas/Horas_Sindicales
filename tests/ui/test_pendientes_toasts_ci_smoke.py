...
def _resolver_directorio_evidencia(tmp_path: Path) -> Path:
    ...
    evidencia_dir = (
        Path(override) if override else Path("artifacts/ui_smoke_evidencias")
    )
...
def _guardar_evidencia(tmp_path: Path, nombre: str, payload: dict[str, object]) -> None:
    ...
    (evidencia_dir / nombre).write_text(
...
def _eventos(caplog: pytest.LogCaptureFixture, prefijo: str) -> list[str]:
    return [
        r.getMessage() for r in caplog.records if r.getMessage().startswith(prefijo)
    ]
...
def test_ui_smoke_pendientes_y_toasts_con_evidencias_ci(
    ...
    monkeypatch.setattr(
        window.notifications, "show_confirmation_closure", _capturar_cierre
    )
    ...
    try:
        ...
        persona_1 = window._persona_use_cases.crear_persona(
            crear_persona_dto_valida("Delegada Smoke Uno")
        )
        persona_2 = window._persona_use_cases.crear_persona(
            crear_persona_dto_valida("Delegada Smoke Dos")
        )
        ...
        _guardar_evidencia(
            ...
            "evidencia_ver_todas_delegadas.json",
            {
                ...
                "ids_relevantes": {
                    "persona_1": persona_1.id,
                    "persona_2": persona_2.id,
                    "pendiente_persona_1": id_p1,
                    "pendiente_persona_2": id_p2,
                },
                "hitos": [
                    "click_ver_todas_delegadas",
                    "recarga_pendientes",
                    "alineacion_visible_logico",
                ],
                "callbacks_detectados": {},
                "razon_fallo": None
                if pass_s1
                else "Quedaron ocultas o los conteos no alinean.",
                "mensaje_humano": "Ver todas delegadas limpia ocultas y alinea tabla con el estado lógico."
                if pass_s1
                else "Persisten ocultas o desalineación de conteos.",
            },
        )
        assert pass_s1
            ...
            return ResultadoConfirmacionFlujo(
                ...
                resultado=SolicitudConfirmarPdfResultado(
                    ...
                    pendientes_restantes=[
                        sol.id
                        for sol in window._pending_solicitudes
        ...
        monkeypatch.setattr(
            window, "_finalize_confirmar_with_pdf", lambda *_args, **_kwargs: None
        )
        ...
        _guardar_evidencia(
            ...
            "evidencia_seleccion_pendientes_post_ver_todas.json",
            {
                ...
                "conteos_despues": {
                    "seleccion_limpia": len(seleccion_limpia),
                    "seleccion_toggle_2": len(seleccion_toggle_2),
                    "seleccion_real_confirmar": len(selected_real),
                },
                "ids_relevantes": {
                    "selection_model_ids": selected_real,
                    "ids_consumidos_confirmar": capturado.get("ids", []),
                },
                "hitos": [
                    "toggle_visible_on_off",
                    "seleccion_real_selection_model",
                    "confirmar_usa_seleccion_real",
                ],
                "callbacks_detectados": {},
                "razon_fallo": None
                if pass_s2
                else "Confirmar/PDF no consumió la misma selección real.",
                "mensaje_humano": "La selección visible/rango se conserva y confirmar consume los IDs reales."
                if pass_s2
                else "Desalineación entre selección visible y payload de confirmar.",
            },
        )
        assert pass_s2
        scenario_status.append(
            {"escenario": "seleccion_post_ver_todas", "estado": "PASS"}
        )
        ...
        success_present = any(
            r.nivel == "success" for r in window.toast._cache.values()
        )
        ...
        _guardar_evidencia(
            ...
            "evidencia_toast_success_real.json",
            {
                ...
                "conteos_despues": {
                    ...
                    "eventos_confirmar": len(eventos_confirmar),
                },
                "ids_relevantes": {
                    "ids_seleccionados": _ids_seleccionados(window),
                    "pdf_guardados": save_calls,
                },
                "hitos": [
                    "flujo_confirmar_valido",
                    "ui_confirmar_pdf_execute_ok",
                    "toast_success_emitido",
                ],
                "callbacks_detectados": callbacks,
                "razon_fallo": None
                if pass_s3
                else "Falta toast success real, callback o apareció error contradictorio.",
                "mensaje_humano": "Flujo válido muestra success coherente sin excepciones de confirmar."
                if pass_s3
                else "Contrato de success no cumplido en flujo válido.",
            },
        )
        assert pass_s3
        ...
        success_toasts = [
            t.id for t in window.toast._cache.values() if t.nivel == "success"
        ]
        error_toasts = [
            t.id for t in window.toast._cache.values() if t.nivel == "error"
        ]
        pass_s4 = (
            "UI_CONFIRMAR_PDF_EXECUTE_ERROR" in eventos_error
            and not success_toasts
            and bool(error_toasts)
        )
        _guardar_evidencia(
            tmp_path,
            "evidencia_toast_error_real.json",
            {
                ...
                "conteos_despues": {
                    "toasts_cache": len(window.toast._cache),
                    "toasts_error": len(error_toasts),
                    "toasts_success": len(success_toasts),
                },
                "ids_relevantes": {
                    "toast_error_ids": error_toasts,
                    "toast_success_ids": success_toasts,
                },
                "hitos": [
                    "error_controlado_execute",
                ...
                "razon_fallo": None
                if pass_s4
                else "Hubo success indebido o faltó toast de error.",
                "mensaje_humano": "Ante fallo real, solo se muestra error coherente y sin éxito falso."
                if pass_s4
                else "Se detectó mensaje contradictorio de success/error.",
            },
        )
        assert pass_s4
        ...
        app.processEvents()
        cierre_manual_hit = (
            target_id not in window.toast._cache
            and target_id not in window.toast._visibles
        )
        ...
        _guardar_evidencia(
            ...
            "evidencia_toast_cierre_manual.json",
            {
                ...
                "razon_fallo": None
                if pass_s5
                else "No se limpiaron cache/modelo/visibles tras cierre manual.",
                "mensaje_humano": "El cierre manual elimina el toast y evita zombis de UI."
                if pass_s5
                else "Cierre manual dejó estado zombi o inconsistente.",
            },
        )
        assert pass_s5
        ...
        _guardar_evidencia(
            ...
            "resumen_ui_pendientes_toasts.json",
            {
                ...
                "estado_global": "PASS"
                if all(s["estado"] == "PASS" for s in scenario_status)
                else "FAIL",
                "mensaje_humano": "Todos los contratos runtime de pendientes/toasts pasan en smoke UI headless."
                if all(s["estado"] == "PASS" for s in scenario_status)
                else "Hay contratos runtime sin cerrar.",
            },
        )
    finally:
        window.close()
