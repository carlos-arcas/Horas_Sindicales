...
|---|---|---|---|---|---|
| FUN-001 | Se puede lanzar la aplicación por entrypoint Python y por launcher de Windows. | Alta | Verificada | `tests/test___main___smoke.py`, `tests/ui/test_ui_smoke_startup.py`, `tests/test_launcher_bat_contract.py` | El doble clic en Windows real no se valida en este entorno Linux; E2E queda parcial. |
...
| FUN-007 | Se puede gestionar el módulo web de personajes (listar/crear/editar/eliminar) en vertical Django en transición. | Media | Parcial | `tests/application/personajes/test_casos_uso_personajes.py`, `tests/domain/personajes/test_servicios_personaje.py`, `tests/web/test_personajes.py`, `docs/features.json` | Vertical marcada como transición/WIP; sin evidencia E2E consolidada. |
