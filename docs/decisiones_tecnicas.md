...
- **2026-03-18 — `objectName` como identidad runtime real del contrato UI read-only — Vigente**  
  Tras convertir `objectName` en identidad principal del inventario persistía una incoherencia técnica: `resolver_control_mutante` todavía aceptaba una degradación genérica por atributo Python si el lookup real de Qt fallaba. Esa compatibilidad silenciosa podía ocultar drift en la UI y debilitaba el contrato auditado.  
  Se endurece `app/ui/vistas/main_window/politica_solo_lectura.py`: `resolver_control_mutante` queda 100% guiado por `findChildren(..., object_name)` y desaparece el fallback genérico `getattr(window, object_name)`. La auditoría smoke/UI valida que todos los controles mutantes auditados —incluido `QAction` del menú demo— se resuelven por `objectName`, y los guardarraíles headless/AST impiden reintroducir helpers de compatibilidad por atributo o una resolución híbrida ambigua.  
  Resultado: el contrato read-only deja de ser “objectName como preferencia” y pasa a ser “objectName como identidad runtime efectiva”, con fallo visible si un control pierde su `objectName` y sin alterar el comportamiento read-only ya validado.
