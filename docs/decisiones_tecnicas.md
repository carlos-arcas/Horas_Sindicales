...
- **2026-03-18 — `objectName` como identidad principal del contrato UI read-only — Vigente**  
  Tras centralizar el inventario mutante auditado quedó una duplicidad residual en los descriptores runtime: `nombre_control` y `object_name` transportaban la misma identidad para todos los controles auditados. Ese doble campo no aportaba semántica adicional real, ampliaba la superficie del contrato y mantenía una resolución híbrida donde el atributo Python seguía pesando más de lo necesario.  
  Se simplifica el contrato de `app/ui/vistas/main_window/politica_solo_lectura.py`: `DescriptorAccionMutante` conserva sólo `object_name`, `tipo_control`, `pantalla` y `accion`; la exportación del inventario queda indexada por `objectName`; y `resolver_control_mutante` pasa a resolver primero por `findChildren(..., object_name)` y sólo usa el atributo homónimo como compatibilidad secundaria, nunca como identidad dominante. Los tests de contrato/headless/smoke se reescriben para expresar el guardarraíl en términos de `objectName` y para impedir la reintroducción de `nombre_control` en runtime.  
  Resultado: contrato read-only más corto, una sola identidad principal observable en UI real, menos riesgo de drift silencioso y misma conducta visible de disable + tooltip.
